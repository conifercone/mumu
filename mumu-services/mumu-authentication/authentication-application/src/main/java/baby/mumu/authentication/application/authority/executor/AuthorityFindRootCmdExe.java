 ...
 */
@Component
@Observed(name = "AuthorityFindRootCmdExe")
public class AuthorityFindRootCmdExe {

  private final AuthorityGateway authorityGateway;
  private final AuthorityConvertor authorityConvertor;

  @Autowired
  public AuthorityFindRootCmdExe(AuthorityGateway authorityGateway,
    AuthorityConvertor authorityConvertor) {
    this.authorityGateway = authorityGateway;
    this.authorityConvertor = authorityConvertor;
  }

  public Page<AuthorityFindRootCo> execute(AuthorityFindRootCmd authorityFindRootCmd) {
    return Optional.ofNullable(authorityFindRootCmd).map(authorityFindRootCmdNotNull -> {
      Page<Authority> authorities = authorityGateway.findRootAuthorities(
        authorityFindRootCmdNotNull.getCurrent(), authorityFindRootCmdNotNull.getPageSize());
      List<AuthorityFindRootCo> authorityFindRootCos = authorities.getContent().stream()
        .map(authorityConvertor::toAuthorityFindRootCo)
        .filter(Optional::isPresent).map(Optional::get).toList();
      return new PageImpl<>(authorityFindRootCos, authorities.getPageable(),
