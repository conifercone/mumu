...
- `build-logic` convention plugin system with `mumu.java-conventions`, Kotlin, and Processor convention plugins
- Protobuf Gradle plugin
- Composite Gradle plugins for service aggregation modules
- Quality check and publishing convention plugins
- Architecture review report
- `CLAUDE.md` and `GEMINI.md` project documentation
- Git configuration files

=== 🕸️ Changed

- `Gradle` upgraded to 9.5.1
- `Spring Boot` upgraded to 4.0.6
- `Kotlin` upgraded to 2.3.21
- `gRPC` upgraded to 1.81.0
- `Protobuf` upgraded to 4.35.0
- `com.palantir.javapoet:javapoet` upgraded to 0.15.0
- `software.amazon.awssdk:bom` upgraded to 2.44.13
- `com.googlecode.libphonenumber:libphonenumber` upgraded to 9.0.31
- `io.swagger.core.v3:swagger-annotations-jakarta` upgraded to 2.2.50
- `io.freefair.lombok` upgraded to 9.5.0
- `com.deepl.api:deepl-java` upgraded to 1.16.0
- `io.minio:minio` upgraded to 9.0.1
- `com.bucket4j:bucket4j_jdk17-lettuce` upgraded to 8.19.0
- `org.jobrunr:jobrunr-spring-boot-4-starter` upgraded to 8.6.0
- `commons-io:commons-io` upgraded to 2.22.0
- `cosid` upgraded to 3.0.5
- `com.redis.om:redis-om-spring` upgraded to 2.0.4
- `org.springdoc:springdoc-openapi-starter-webmvc-ui` upgraded to 3.0.3
- `io.hypersistence:hypersistence-utils-hibernate-71` upgraded to 3.15.2
- `com.gradle.develocity` upgraded to 4.4.1
- `com.google.protobuf` upgraded to 0.10.0
- `org.apache.tika:tika-core` upgraded to 3.3.0
- `org.springframework.grpc:spring-grpc-dependencies` upgraded to 1.0.3
- `io.opentelemetry.instrumentation:opentelemetry-log4j-appender-2.17` upgraded to 2.28.1-alpha
- IAM layer refactored: application layer no longer depends on infrastructure
- IAM `AccountConvertor` and `RoleConvertor` refactored to use MapStruct mappers
- Enforce internal MapStruct mapper usage
- Gradle convention plugins refactored for type-safe `libs` accessors
- Timezone handling principle: frontend-localized, backend-unified
- Build scripts optimized and README updated

=== 🐞 Fixed

- Fixed `compileJava` task compatibility with configuration cache
- Fixed unresolved `libs` reference in convention plugins
- Restored missing `SpringBoot` configuration in service modules
- Fixed compatibility with new MinIO dependency

== [2.16.0] - 2026-02-11

=== ⭐ Added

- `log4j2` integrates `otel`
- Add `move` interface to permission management
- Add `codex` file
- Permission management adds a new interface to obtain all ancestor paths of a specified descendant node
...
- `Spring Boot` upgraded to version 4.x
- `JDK` upgraded to 25
- `com.palantir.javapoet:javapoet` upgraded to 0.11.0
- `software.amazon.awssdk:bom` upgraded to 2.41.24
- `grpc` upgraded to 1.79.0
- `org.jobrunr:jobrunr-spring-boot-4-starter` upgraded to 8.4.2
- `Kotlin` upgraded to 2.3.10
- `com.bucket4j:bucket4j_jdk17-lettuce` upgraded to 8.16.1
- `com.googlecode.libphonenumber:libphonenumber` upgraded to 9.0.23
- `protobuf` upgraded to 4.33.5
- `io.hypersistence:hypersistence-utils-hibernate-71` upgraded to 3.15.1
- `io.swagger.core.v3:swagger-annotations-jakarta` upgraded to 2.2.42
- `net.sourceforge.tess4j:tess4j` upgraded to 5.18.0
- `me.tongfei:progressbar` upgraded to 0.10.2
- `io.freefair.lombok` upgraded to 9.2.0
- `org.springdoc:springdoc-openapi-starter-webmvc-ui` upgraded to 3.0.1
- `com.deepl.api:deepl-java` upgraded to 1.14.0
- `org.apache.commons:commons-text` upgraded to 1.15.0
- Do not print `jobrunr` related sql to reduce log noise
- `indent_size` modified to 4
- Optimize pmd configuration file

=== 🐞 Fixed

- Fixed the problem of failure to add permissions
- Fixed signature verification failure issue
...
- Optimize global link tracking ID acquisition logic
- `Gradle` upgraded to 9.2.1
- `org.jobrunr:jobrunr-spring-boot-3-starter` upgraded to 8.2.0
- `com.googlecode.libphonenumber:libphonenumber` upgraded to 9.0.18
- `org.springdoc:springdoc-openapi-starter-webmvc-ui` upgraded to 2.8.14
- `commons-io:commons-io` upgraded to 2.21.0
- `io.hypersistence:hypersistence-utils-hibernate-63` upgraded to 3.12.0
- `flyway` upgraded to 11.16.0
- `io.freefair.lombok` upgraded to 9.1.0
- `com.deepl.api:deepl-java` upgraded to 1.11.0
- `io.swagger.core.v3:swagger-annotations-jakarta` upgraded to 2.2.40
- `org.springframework.grpc:spring-grpc-dependencies` upgraded to 0.12.0
- `Spring Boot` upgraded to 3.5.7
- `kotlin` upgraded to 2.2.21
- `protobuf` upgraded to 4.33.0
- `io.minio:minio` upgraded to 8.6.0
- `com.google.guava:guava-bom` upgraded to 33.5.0-jre
- `org.apache.tika:tika-core` upgraded to 3.2.3
- `com.redis.om:redis-om-spring` upgraded to 1.0.4
- Improve sql log printing performance
- Optimize account deletion logic
- Optimize multi-language prompt information
- Optimize build script
- After the business logic is successful, the verification code is deleted and changed to asynchronous deletion.
- Modify the return value type of the verification code generation method
- Verification code related renaming
- `unique` service renamed to `genix`
- Optimize the general method of obtaining IP
- Refactor class name
- Disable Jackson's WRITE_DATES_AS_TIMESTAMPS attribute
- Modify the creation method of Jackson Mapper
- Optimize file writing method
- Modify the field name generation rules in the annotation processor

=== ⚠️ Removed

- Remove undertow and use tomcat instead
...
- Detect if the avatar file needs to be deleted when deleting an account.
- Integrate jspecify
...
- grpc dependency replacement with spring-grpc
- Replacing constant characters
- Modify the corresponding exception category in the system log
- Replace the domain name of the link address of the dependency description
- Optimize the logic of obtaining the true type of file
- Optimize the conditions for global response interceptors to take effect
- Optimize permission query performance
- `org.apache.zookeeper:zookeeper` Upgrade to 3.9.4.
- `com.googlecode.libphonenumber:libphonenumber` Updated to 9.0.13.
- `org.springdoc:springdoc-openapi-starter-webmvc-ui` upgrade to 2.8.13
- `kotlin` upgraded to 2.2.10
- `com.deepl.api:deepl-java` upgraded to 1.10.3
- `com.bucket4j:bucket4j_jdk17-lettuce` Upgraded to 8.15.0.
- `flyway` upgraded to 11.12.0
- `protobuf` upgraded to 4.32.0
- `grpc` upgraded to 1.75.0
- `io.freefair.lombok` upgraded to 8.14.2
- `io.swagger.core.v3:swagger-annotations-jakarta` upgrade to 2.2.36
- `spring boot` upgrade to 3.5.5
- `com.aliyun:alimt20181012` upgrade to 1.5.1
- `com.redis.om:redis-om-spring` upgraded to 1.0.1.
- `org.apache.tika:tika-core` upgrade to 3.2.2
- `commons-io:commons-io` Upgraded to 2.20.0.
- `com.opencsv:opencsv` upgrade to 5.12.0
- `org.jobrunr:jobrunr-spring-boot-3-starter` upgraded to 8.0.2
